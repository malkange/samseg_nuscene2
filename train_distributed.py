...
def main():
    ...
    if datasetname == 'SemanticKitti':
        ...
        mini_set = cfgs['dataset']['mini_set']
        train_pt_dataset = Nuscenes_pt(data_path, split='train', cfgs=cfgs, nusc=nusc, version=version, mini_set=mini_set)
        val_pt_dataset = Nuscenes_pt(data_path, split='val', cfgs=cfgs, nusc=nusc, version=version, mini_set=mini_set)
    else:
        ...
        if (epoch > 0):
            ...
            with torch.no_grad():
                for i_iter_val, val_dict in enumerate(val_dataset_loader):
                    ...
                    val_pt_labels = val_dict['pt_sem_label'] # (4,)
                    val_pt_inst = val_dict['pt_ins_label'] # (4,)
