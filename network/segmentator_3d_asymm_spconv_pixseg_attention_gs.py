...
class PositionwiseFeedforwardLayer(nn.Module):
    ...
    def forward(self, x):
        # x : [batch_size, 3, hidden_dim)
        x = self.drop_out(torch.relu(self.fc_1(x)))
        # x : [batch_size, 3, pf_dim]
        x = self.fc_2(x)
        # x : [batch_size, 3, hidden_dim)
        return x
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src):
        ...
        # self attention
        _src = self.self_attention(src, src, src)[0]
...
class TransformerEncoder(nn.Module):
    def __init__(self, hidden_dim, n_layers, n_heads, pf_dim, dropout_ratio):
        super().__init__()
        self.layers = nn.ModuleList([TransformerEncoderLayer(hidden_dim, n_heads, pf_dim, dropout_ratio) for _ in range(n_layers)])
        self.dropout = nn.Dropout(dropout_ratio)
...
class Asymm_3d_spconv(nn.Module):
    def __init__(self,
        ...
        #self.attention = TransformerEncoder(hidden_dim = 64, n_layers = 3, n_heads = 4, pf_dim = 128, dropout_ratio = 0.1)
        self.attention = TransformerEncoder(hidden_dim=64, n_layers=1, n_heads=1, pf_dim=128, dropout_ratio=0.1)
    ...
    def forward(self, voxel_features, coors, batch_size, pooled):
        ...
        ###
        ret_seg = self.downCntxSeg(ret_seg)
        ret_pix = self.downCntxPix(ret_pix)
        down_seg, down_seg2 = self.resSeg(ret_seg)  # [127899, 64]
        ...
        """ gs : pix concat """
        # cat_fea = torch.cat((up2e.features[:, None, :], down_pix.features[:, None, :]), 1)  # [127899, 2, 64]
        # up2e = up2e.replace_feature(cat_fea.reshape((-1, 64 * 2)))  # [127899, 64*2]
        # down1b = down1b + down_pix2  # [127899, 64]
        # scomp0 = up2e  # [127899, 128]

        """ gs : seg concat """
        # cat_fea = torch.cat((up2e.features[:, None, :], down_seg.features[:, None, :]), 1)  # [127899, 2, 64]
        # up2e = up2e.replace_feature(cat_fea.reshape((-1, 64 * 2)))  # [127899, 64*2]
        # down1b = down1b + down_seg2  # [127899, 64]
        # scomp0 = up2e  # [127899, 128]

        """ gs : pix + seg concat """
        #cat_fea = torch.cat((up2e.features[:, None, :], down_seg.features[:, None, :], down_pix.features[:, None, :]),
        #                    1)  # [127899, 3, 64]
        #up2e = up2e.replace_feature(cat_fea.reshape((-1, 64 * 3)))  # [127899, 64*3]
        #down1b = down1b + down_seg2 + down_pix2  # [127899, 64]
        #scomp0 = self.secon_compress0(up2e)  # [127899, 128]

        """ gs attention based fusion start """
        cat_fea = torch.cat((up2e.features[:, None, :], down_seg.features[:, None, :], down_pix.features[:, None, :]),
                            1)  # [127899, 3, 64]
        attn_fea = self.attention(cat_fea)
        up2e = up2e.replace_feature(attn_fea.reshape((-1, 64 * 3)))  # [127899, 64*3]
        down1b = down1b + down_seg2 + down_pix2 # [127899, 64]
        scomp0 = self.secon_compress0(up2e)  # [127899, 128]
        """ gs end """
