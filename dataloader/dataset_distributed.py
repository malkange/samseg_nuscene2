...
class SemKITTI_pt(data.Dataset):
    def __init__(self, data_path, cfgs, split='train', return_ref=False):
        ...
        # gh
        # /home/data/SemanticKitti/
        self.root_sam = data_path.replace('SemanticKitti', 'SemKITTI_processed')  #'/home/poscoict/Desktop/SemKITTI_processed/dataset/sequences'
        ...
        # sequences
        if self.split == 'train':
            self.seqs =['00', '01', '02', '03', '04', '05', '06', '07', '09']  #'03',
        elif self.split == 'val':
            self.seqs =  ['08', '10']
        ...
        self.sam_segidx_files = []
        self.sam_imgidx_files = []
        ...
        for seq in self.seqs:
            path_list = os.listdir(os.path.join(self.root, seq, 'velodyne'))
            path_list = sorted(path_list)
            mask_ = np.zeros(len(path_list), dtype=bool)
            ids = np.linspace(0, len(path_list), int(len(path_list) * 0.1), endpoint=False, dtype=int)
            mask_[ids] = True
            path_list = np.array(path_list)[mask_]
            for pcd_name in sorted(path_list):
                ...
                self.sam_imgidx_files.append(os.path.join(self.root_sam, seq, 'img_fea', str(pcd_name[:-4]) + '_idx.pt'))
                self.sam_segidx_files.append(os.path.join(self.root_sam, seq, 'seg_fea', str(pcd_name[:-4]) + '_idx.pt'))
    ...
    def _load_sam(self, index):
        ...
        imgidx_path = self.sam_imgidx_files[index]
        segidx_path = self.sam_segidx_files[index]
        imgfea = torch.load(imgfea_path, map_location='cpu')
        pixloc = torch.load(imgidx_path, map_location='cpu').to(torch.long)
        segfea = torch.load(segfea_path, map_location='cpu')
        segidx = torch.load(segidx_path, map_location='cpu').to(torch.long)

        return imgfea, pixloc, segfea, segidx
    ...
    def __getitem__(self, index):
        ...
        # gh
        imgfea_sub, pix_loc, segfea, segidx = self._load_sam(index)
        ...
        segfea_ = torch.zeros((len(segfea)+1, 256))
        segfea_[:-1] = segfea
        segidx[segidx==255] = len(segfea)

        xyz_segfea, xyz_pixfea = torch.zeros((len(pts), 3, 256)), torch.zeros((len(pts), 256))
        # breakpoint()
        # if mask1.sum()!=len(segidx.T):
        #     print(self.sam_imgfea_files[index])
        #     breakpoint()
        xyz_segfea[mask1] = segfea_[segidx].transpose(1,0)

        # segidx segfea mask1
        # breakpoint()
        imgfea = torch.zeros((256, 64,64), dtype=torch.float32)
        imgfea[:, pix_loc[0], pix_loc[1]] = imgfea_sub.to(torch.float32)



        # get pixel feature
        xyz_pixfea = torch.zeros((len(pts), 256), dtype=torch.float32)
        uvs = pixel_coordinates1[mask1]

        grid_tmp = torch.Tensor(uvs[None, :,None, :]) #.cuda() # (1, n_pts, 1, 2)
        xyz_pixfea_sub = torch.nn.functional.grid_sample(imgfea[None, ...], grid_tmp)[0, ..., 0].transpose(1,0)
        xyz_pixfea[mask1] = xyz_pixfea_sub
        ...
        # gh
        fusion_tuple = (xyz_segfea, xyz_pixfea)  # gh todo not mono?
...
class Nuscenes_pt(data.Dataset):
    def __init__(self, data_path, split, cfgs, nusc, version, assync_compensation=True, mini_set = False):
        ...
        if self.mini :
            ...
            for i in range(len(self.nusc_infos)):
                info = self.nusc_infos[i]
                cam_token = info['cams'][self.CAM_CHANNELS[0]]['sample_data_token']
                cam_channel = self.nusc.get('sample_data', cam_token)
                
                fn_list.append(cam_channel['filename'][18:42])
            fn_np = np.array(fn_list)   
            fn_np_uni = np.unique(fn_np)
            nusc_info_old = self.nusc_infos
            self.nusc_infos = []
            for uni_ele in fn_np_uni:
                tar_bool = (fn_np == uni_ele)
                num = int(np.ceil(tar_bool.sum() * 0.1))
                tar_bool[tar_bool.cumsum()>num] = False
                
                for i in np.arange(tar_bool.shape[0])[tar_bool]:
                    self.nusc_infos.append(nusc_info_old[i])
            # breakpoint()
    ...
    # gh
    def _load_sam(self, channel, file_id):

        dst_seg = os.path.join(self.data_path, 'seg_fea')
        dst_img = os.path.join(self.data_path, 'img_fea')

        imgfea = torch.load(os.path.join(dst_img, channel, file_id + ".pt"), map_location='cpu')
        segfea = torch.load(os.path.join(dst_seg, channel, file_id + ".pt"), map_location='cpu')
        pixloc = torch.load(os.path.join(dst_img, channel, file_id + "_idx.pt"), map_location='cpu').to(torch.long)
        segidx = torch.load(os.path.join(dst_seg, channel, file_id + "_idx.pt"), map_location='cpu').to(torch.long)
    ...
    def __getitem__(self, index):
        ...
        points_ids = torch.Tensor(np.linspace(0, len(points) - 1, len(points)).astype(int)).to(torch.long)

        file_id = lidar_path.split('/')[-1].split('.')[0]
        ...
        total_xyz_segfea = torch.zeros((len(points), 3, 256))
        total_xyz_pixfea = torch.zeros((len(points), 256))
        total_xyz_segscore = torch.zeros((len(points), 3))
        count_xyz = torch.zeros((len(points),))
        for idx, channel in enumerate(self.CAM_CHANNELS):
            ...
            imgfea_sub, pix_loc, segfea, segidx = self._load_sam(channel, file_id)

            
            
            # sub_pts_t = points[:, :3][mask]
            
            # pts_t = torch.Tensor(sub_pts_t)
            ...
            # xyz_segfea, xyz_pixfea = torch.zeros((len(pts_t), 3, 256)), torch.zeros((len(pts_t), 256))
            
            
            xyz_segfea = segfea_[segidx].transpose(1, 0)
                

            imgfea = torch.zeros((256, 64, 64), dtype=torch.float32)
            imgfea[:, pix_loc[0], pix_loc[1]] = imgfea_sub.to(torch.float32)
            
            # xyz_pixfea = torch.zeros((len(pts_t), 256), dtype=torch.float32)
            uvs = pixel_coord[:,mask].astype(np.int32).T

            grid_tmp = torch.Tensor(uvs[None, :, None, :])  # (1, n_pts, 1, 2)
            
            xyz_pixfea_sub = torch.nn.functional.grid_sample(imgfea[None, ...], grid_tmp)[0, ..., 0].transpose(1,0)
            xyz_pixfea = xyz_pixfea_sub
            
            # (sj) condier mask and counting
            count_xyz[mask] = count_xyz[mask] + 1
            total_xyz_segfea[mask] = total_xyz_segfea[mask] + xyz_segfea
            total_xyz_pixfea[mask] = total_xyz_pixfea[mask] + xyz_pixfea
            
            print((xyz_pixfea.sum(-1)==0).sum())
            # total_xyz_segscore[mask] = total_xyz_segscore[mask] + xyz_segscore
        ...
        total_xyz_pixfea = total_xyz_pixfea / torch.clamp(count_xyz, 1.0)[:, None]
        total_xyz_segfea = total_xyz_segfea / torch.clamp(count_xyz, 1.0)[:, None, None]

        
        fusion_tuple = (total_xyz_segfea, total_xyz_pixfea)
...
class spherical_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # gh
        return_dict['segfea'] = fusion_tuple[0] # [127000, 3, 256]
        if self.attn is False:
            ...
            # gs
            return_dict['segfea'] = fusion_tuple[0].mean(1) # [127000, 256]
            # return_dict['segfea'] = fusion_tuple[0]  # [127000, 3, 256]
